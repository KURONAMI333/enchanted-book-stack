 ...
 * PEB の司書交易追加。
 *
 * <p>vanilla {@code librarian} (司書) villager の Level 4 (Expert) trade list に PEB を追加。
 *
 * <p>取引内容: <b>24 emerald + 4 leather</b> ↔ <b>Portable Enchanted Bookshelf ×1</b>
 *
 * <p>価格設計の根拠:
 * <ul>
 ...
 * </ul>
 */
public final class PEBTrades {

    private PEBTrades() {}

    /** {@code NeoForge.EVENT_BUS.addListener} で登録される。 */
    public static void onRegisterTrades(VillagerTradesEvent event) {
        if (event.getType() == VillagerProfession.LIBRARIAN) {
            // Level 4 (Expert) trades
            event.getTrades().get(4).add(new PEBTradeListing());
        }
    }
     ...
     */
    private static final class PEBTradeListing implements VillagerTrades.ItemListing {
        @Override
        public MerchantOffer getOffer(Entity trader, RandomSource random) {
            ItemStack peb = new ItemStack(PEBItems.PORTABLE_ENCHANTED_BOOKSHELF.get(), 1);
