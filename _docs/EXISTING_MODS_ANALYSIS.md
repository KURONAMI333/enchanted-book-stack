...
- **判定**: **着工 GO (条件付き)**
  ...
  - **1.20.1 Forge 対応** は Apotheosis (122M DL) が同領域を block ベースで完全カバーしているため、KURONAMI333 のターゲット層 (司書ファーム民) のうち Apotheosis 派には届かない可能性。ただし vanilla 派 / Apotheosis 未導入派には十分需要あり
...
7. **tree 階層 GUI** (KURONAMI333 独自 UX) vs grid + 横一覧
...
- 「物理本維持」 vs 「ポイント変換」は思想差。KURONAMI333 の SPEC は物理本維持 → 司書交易で得た本を「そのまま渡せる/取り出せる」UX 重視
...
| 最終更新 | 2026-03-12 (Modrinth では極めて最近に表示、CF は 13 年塩漬けと KURONAMI333 メモ) |
| MC / Loader | 1.12.2 Forge only |

**歴史的存在として記録。1.21 / 1.20 ユーザーには無関係。**
ただし「Modrinth に 2026-03 表示」は版上げ無しの再アップロード可能性が高い (要 GitHub commit 確認)。KURONAMI333 メモの「13 年塩漬け」と表示更新日のズレは「公式 PR の merge 履歴は source の延長」原則に抵触しない範囲 (1.12.2 から動かしていない)。**警戒度ゼロ**。
...
| Limitless Enchantments | 43,000 (KURONAMI333 メモ) | 不明 | level cap 撤廃のみ、保管解決せず |
...
**4 tier upgrade UX 規範 (KURONAMI333 仕様の元ネタ)**:
...
| Mojang vanilla Bundle | 「全 item 共通容量公式」維持 → エンチャ本特化はやらない (KURONAMI333 SPEC 通り) | **極低** |
...
## 補足: KURONAMI333 のサクッと検索を補強した発見

KURONAMI333 メモの「Modrinth + CurseForge で 1.21 類似 0 件」の見落とし:

1. **Enchanted BookShelf (jorgej_ramos, CF 225 DL, 1.21.1 NeoForge)** — KURONAMI333 検索で見落とし。コンセプトはほぼ同じ (block / 1000 冊 / 検索 GUI / enchant index)。**最重要警戒対象**。
2. **Enchantment Library at Home (CF 1.1M DL, 1.20.1 Forge)** — KURONAMI333 メモにない。1.20.1 帯ではデファクト。思想は point 変換式で住み分け可能だが、ターゲット層の一部 (Apotheosis 派) は既にこれで満足している可能性。
3. **Enchantment Library Standalone (Sotnah, Modrinth 256 / CF 496)** — KURONAMI333 メモにない。Apothic から分離した 1.21.1 NeoForge 版、新規だが低 DL。
